...
// v8.9: Corrected window.location.href for login to use Flask route /login.
// v8.8: Modified modal close logic to prevent 'Add New Project' modal from closing on backdrop click.
...
async function apiFetch(url, options = {}) {
    ...
    try {
        ...
        if (response.status === 401) {
            ...
            window.location.href = '/login'; // Corrected to Flask route
            console.groupEnd();
...
function renderProjectTable(dataToRender, tableBody, isCompletedTable = false) {
    ...
    try {
        dataToRender.forEach((project) => {
            ...
            const mrfLink = `/mrf_form.html?${mrfParams}`; // Corrected to Flask route
            ...
            tr.innerHTML = `
                ...
                <td class="px-2 py-1 border-b border-gray-200 dark:border-slate-700 text-sm w-auto action-column space-x-1 flex flex-wrap items-center">
                    ...
                    <a href="/project_gantt.html?project_id=${project.id}" class="${buttonBaseClass} text-gray-600 hover:text-gray-800 dark:text-slate-400 dark:hover:text-slate-300 focus:ring-gray-300 dark:focus:ring-slate-600 inline-flex items-center" title="View Gantt Chart" target="_blank">${svgChartBar}</a>
                </td>
            `;
        ...
        try {
            dataToRender.sort((a, b) => {
                ...
                if (key === 'latest_update') {
                    ...
                    if (valA && !valB) return -1; // Corrected from !textB to !valB
...
function openAddProjectModal() {
    ...
    getElement('addProjectName')?.focus();
...
function openEditModal(projectId) {
    ...
    getElement('editProjectName').value = project.project_name || '';
    getElement('editProjectNo').value = project.project_no || '';
    getElement('editClient').value = project.client || '';
    getElement('editAmount').value = project.amount ?? '';
    getElement('editStatus').value = project.status ?? '';
    getElement('editPoDate').value = project.po_date ? project.po_date.split('T')[0] : '';
    getElement('editDateCompleted').value = project.date_completed ? project.date_completed.split('T')[0] : '';
    getElement('editPic').value = project.pic || '';
    editProjectModal.classList.remove('hidden');
    editProjectModal.classList.add('flex');
    getElement('editProjectName')?.focus();
...
async function openProjectDetailsModal(projectId) {
    ...
    console.log("    - Found project data:", project);
    getElement('projectModalTitle').textContent = project.project_name || 'N/A';
    getElement('projectModalSubtitle').textContent = `Project #${project.project_no || 'N/A'}`;
    getElement('projectDetailClient').textContent = project.client || 'N/A';
    getElement('projectDetailAmount').textContent = formatCurrency(project.amount);
    getElement('projectDetailStatus').textContent = formatPercent(project.status);
    getElement('projectDetailPoDate').textContent = formatDate(project.po_date);
    getElement('projectDetailPic').textContent = project.pic || 'N/A';
    getElement('projectDetailRunningTime').textContent = `${project.total_running_weeks || 0} weeks`;
    console.log("    - Static details populated.");
    const addUpdateForm = getElement('addUpdateForm');
    const addUpdateMessageEl = getElement('addUpdateMessage'); // Corrected variable name
    if (addUpdateForm) {
        addUpdateForm.dataset.projectId = project.id;
        addUpdateForm.reset();
        if (addUpdateMessageEl) addUpdateMessageEl.textContent = ''; // Use corrected variable
...
function openForecastModal(projectId) {
    ...
    const forecastDateInput = getElement('forecast-date'); // This is the combined hidden one
    const forecastMonthSelect = getElement('forecast-month');
    const forecastYearInput = getElement('forecast-year-input');
    ...
    if (!forecastModal || !forecastForm || !forecastProjectIdInput || !forecastProjectInfoSpan ||
        !forecastDateInput || !forecastMonthSelect || !forecastYearInput ||
        !forecastAmountInput || !forecastPercentInput || !forecastDeductCheckbox) {
    ...
    // Set default month and year for the dropdowns
    const today = new Date();
    forecastMonthSelect.value = today.getMonth() + 1;
    forecastYearInput.value = today.getFullYear();
    // Trigger update for the hidden combined date field
    if (typeof updateCombinedDate === 'function') { // Check if defined in HTML inline script
        updateCombinedDate();
    } else { // Fallback if not defined (should be defined in HTML)
        forecastDateInput.value = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-01`;
    }
...
async function handleAddProjectSubmit(event) {
    ...
    if (!projectName) {
        ...
        getElement('addProjectName')?.classList.add('is-invalid'); // Corrected ID
        isValid = false;
        console.warn("    - Validation failed: Project Name required.");
    } else {
        getElement('addProjectName')?.classList.remove('is-invalid'); // Corrected ID
    }
    if (!isValid) {
        console.groupEnd();
        return;
    ...
    projectData.year = safe_int(projectData.year, null); // Assuming year might be added
    // po_no is already included by formData.forEach
...
async function handleEditSubmit(event) {
    ...
    const updatedFields = {};
    let isValid = true;
    // Ensure 'po_no' is included if it's part of the edit form
    ...
    if (updatedFields.project_name === null || updatedFields.project_name === '') { displayFeedback("Project Name cannot be empty.", 'error'); getElement('editProjectName')?.classList.add('is-invalid'); isValid = false; console.warn("    - Validation failed: Project Name empty."); } // Corrected ID
    else { getElement('editProjectName')?.classList.remove('is-invalid'); } // Corrected ID
    if ('amount' in updatedFields) {
        if (updatedFields.amount !== null) {
            const numAmount = safeFloat(updatedFields.amount);
            if (isNaN(numAmount)) { displayFeedback("Invalid Amount.", 'error'); isValid = false; getElement('editAmount')?.classList.add('is-invalid'); console.warn("    - Validation failed: Invalid Amount."); }
            else { updatedFields.amount = numAmount; getElement('editAmount')?.classList.remove('is-invalid'); }
        } else { getElement('editAmount')?.classList.remove('is-invalid'); }
    }
    if ('status' in updatedFields) {
        if (updatedFields.status !== null) {
            const numStatus = safeFloat(updatedFields.status);
            if (isNaN(numStatus) || numStatus < 0 || numStatus > 100) { displayFeedback("Status must be 0-100.", 'error'); isValid = false; getElement('editStatus')?.classList.add('is-invalid'); console.warn("    - Validation failed: Invalid Status."); }
            else { updatedFields.status = numStatus; getElement('editStatus')?.classList.remove('is-invalid'); }
        } else { updatedFields.status = 0.0; getElement('editStatus')?.classList.remove('is-invalid'); }
    }
    if ('year' in updatedFields) { // Assuming year might be part of edit form
        if (updatedFields.year !== null) {
            const numYear = safe_int(updatedFields.year);
            if (numYear === null) { displayFeedback("Invalid Year.", 'error'); isValid = false; getElement('editYear')?.classList.add('is-invalid'); console.warn("    - Validation failed: Invalid Year."); } // Corrected ID if exists
            else { updatedFields.year = numYear; getElement('editYear')?.classList.remove('is-invalid'); } // Corrected ID
        } else { getElement('editYear')?.classList.remove('is-invalid'); } // Corrected ID
    }
    if (updatedFields.po_date && !parseDate(updatedFields.po_date)) { displayFeedback("Invalid PO Date format.", 'error'); isValid = false; getElement('editPoDate')?.classList.add('is-invalid'); console.warn("    - Validation failed: Invalid PO Date."); }
    else { getElement('editPoDate')?.classList.remove('is-invalid'); }
    if (updatedFields.date_completed && !parseDate(updatedFields.date_completed)) { displayFeedback("Invalid Date Completed format.", 'error'); isValid = false; getElement('editDateCompleted')?.classList.add('is-invalid'); console.warn("    - Validation failed: Invalid Date Completed."); }
    else { getElement('editDateCompleted')?.classList.remove('is-invalid'); }
    if (!isValid) {
        console.groupEnd();
        return;
    }
...
async function handleAddUpdateSubmit(event) {
    ...
    event.preventDefault();
    const addUpdateForm = getElement('addUpdateForm');
    const addUpdateMessageEl = getElement('addUpdateMessage'); // Corrected variable name for the message element
    if (!addUpdateForm) {
    ...
    const updateText = addUpdateForm.elements['message']?.value.trim(); // Changed from 'update_text' to 'message' to match HTML
    const dueDate = addUpdateForm.elements['update_due_date']?.value; // Assuming 'update_due_date' is correct in HTML
    ...
    if (!updateText) {
        ...
        addUpdateForm.elements['message']?.classList.add('is-invalid'); // Changed from 'update_text'
        console.warn("    - Validation failed: Update text empty.");
        console.groupEnd();
        return;
    } else {
        addUpdateForm.elements['message']?.classList.remove('is-invalid'); // Changed from 'update_text'
    }
    ...
    const updateData = { update_text: updateText, due_date: validatedDueDate }; // Backend expects update_text
    ...
    if (submitButton) submitButton.disabled = true;
    if (addUpdateMessageEl) { addUpdateMessageEl.textContent = 'Adding...'; addUpdateMessageEl.className = 'message info text-xs mt-1'; }
    try {
        const result = await apiFetch(PROJECT_UPDATES_API(projectId), { method: 'POST', body: updateData });
        if (addUpdateMessageEl) addUpdateMessageEl.textContent = '';
        addUpdateForm.reset();
        ...
        if (error.message !== 'Unauthorized') {
            ...
            if (addUpdateMessageEl) { addUpdateMessageEl.textContent = errorMsg; addUpdateMessageEl.className = 'message error text-xs mt-1 text-red-600 dark:text-red-400'; }
...
async function handleAddForecastSubmit(event) {
    ...
    const forecastAmountInput = getElement('forecast-amount');
    const forecastPercentInput = getElement('forecast-percent');
    const forecastDateInput = getElement('forecast-date'); // This is the combined hidden date
    const forecastDeductCheckbox = getElement('forecast-deduct-checkbox');
    const forecastErrorMessageEl = getElement('forecast-error-message');
    ...
    const dateValue = forecastDateInput.value; // Use the value from the hidden combined field
    ...
    // No need to validate forecastDateInput directly as it's derived
    // getElement('forecast-month').classList.remove('is-invalid'); // If you add validation for month/year
    // getElement('forecast-year-input').classList.remove('is-invalid');
    ...
    if (!dateValue) { // Check the combined date value
        if (forecastErrorMessageEl) forecastErrorMessageEl.textContent = 'Error: Month and Year required.';
        // Optionally mark month/year inputs as invalid
        // getElement('forecast-month').classList.add('is-invalid');
        // getElement('forecast-year-input').classList.add('is-invalid');
        isValid = false; console.warn("    - Validation failed: Date (from month/year) required.");
    } else if (!parseDate(dateValue)) { // Validate the combined date
        if (forecastErrorMessageEl) forecastErrorMessageEl.textContent = 'Error: Invalid Date format from Month/Year.';
        isValid = false; console.warn("    - Validation failed: Invalid Date format from month/year.");
    }
    ...
    if (isDeductionChecked) {
        // Assuming deduction logic might be more complex or configurable,
        // for now, it uses a hardcoded 30% if the checkbox is checked.
        // This part might need adjustment based on actual business rules.
        const deductionPercentageInput = getElement('forecast-deduction-percent');
        let deductionRate = 0.30; // Default if not specified or invalid
        if (deductionPercentageInput && deductionPercentageInput.value) {
            const customRate = safeFloat(deductionPercentageInput.value);
            if (!isNaN(customRate) && customRate >= 0 && customRate <= 100) {
                deductionRate = customRate / 100;
            }
        }
        final_value_to_send = original_input_value * (1 - deductionRate); // Calculate net amount
        console.log(`    - Deduction Calculation (${forecast_input_type}): Original ${original_input_value}, Rate ${deductionRate * 100}%, Sending Net: ${final_value_to_send}`);
    } else {
    ...
    const forecastData = {
        ...
        forecast_date: dateValue, // Send the yyyy-MM-dd formatted date
        is_deduction: final_is_deduction_flag
        // 'invoiced' field is not explicitly sent here, assuming backend handles it or it's part of a different flow
    };
...
async function handleCsvUpload(event) {
    ...
    formData.append('csv-file', file); // Ensure key matches backend: 'csv-file' or 'file'
...
async function handleTableAction(event) {
    ...
    // Prevent default for non-navigation actions that are links
    if (button.tagName === 'A' && action && action !== 'view-gantt' && action !== 'create-mrf') {
        event.preventDefault();
        ...
        if (action === 'create-mrf') {
            // For MRF, we want the browser to navigate.
            // The link in renderProjectTable is now /mrf_form.html?params...
            // This will be served by Flask from the static folder.
            return;
        }
    }
            ...
            case 'delete-update':
                if (!isNaN(updateId)) {
                    ...
                    if (confirm('Are you sure you want to delete this update permanently?')) {
                        ...
                        console.log("    - Update deletion cancelled by user.");
                    }
...
async function deleteProject(projectId, projectName) {
    // Confirmation is now handled in handleTableAction
...
function cancelEdit(inputElement, cell, originalDisplayValue) {
    ...
    if (!cell || cell !== currentlyEditingCell) {
        console.warn("    - Cancel aborted: Cell is no longer the active editing cell.");
        console.groupEnd();
        return;
    }
    const listeners = cell._inlineEditListeners;
...
function exportToCSV(dataToExport, filenamePrefix) {
    ...
    try {
        ...
        } else if (navigator.msSaveBlob) { // For IE
...
function toggleNavigation() {
    ...
    try {
        ...
        // Re-apply nav state which also handles main content margin and avatar visibility
        if (typeof applyInitialNavState === 'function') applyInitialNavState();
...
function toggleTheme() {
    ...
    try {
        ...
        // Update sidebar theme icons if they exist (from inline script)
        if (typeof updateThemeIconsSidebar === 'function') updateThemeIconsSidebar();
    } catch (e) {
...
function applyInitialNavState(sideNavElement) { // sideNavElement is passed from inline script
    console.groupCollapsed("UI: Apply Initial Nav State");
    const localSideNav = sideNavElement || getElement('side-nav'); // Use passed or get it
    const mainContent = getElement('main-content');

    if (!localSideNav || !mainContent) {
    ...
    try {
        if (localStorage.getItem('navCollapsed') === 'true') {
            localSideNav.classList.add('nav-collapsed');
            if (window.innerWidth > 768) mainContent.style.marginLeft = '4rem'; else mainContent.style.marginLeft = '0';
            console.log("    - Applying collapsed state from localStorage.");
        } else {
            localSideNav.classList.remove('nav-collapsed');
            if (window.innerWidth > 768) mainContent.style.marginLeft = localSideNav.offsetWidth + 'px'; else mainContent.style.marginLeft = '0';
            console.log("    - Applying expanded state from localStorage (or default).");
        }
        // Trigger avatar update if function exists (from inline script)
        ...
        console.warn("    - Could not read/apply nav state from localStorage:", e);
    }
    console.groupEnd();
}
...
function setupCollapsibleSections() {
    ...
    console.log("    - Collapsible section listener setup on document.");
    console.groupEnd();
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sideNav = getElement('side-nav');
    const navToggleBtn = getElement('nav-toggle-btn'); // Main nav toggle in script.js
    const settingsToggleBtn = getElement('settings-toggle-btn');
    // const settingsSubMenu = getElement('settings-submenu'); // Already handled by its toggle
    const dashboardLoadingEl = getElement('dashboardLoading');
    const dashboardErrorEl = getElement('dashboardError');
    const activeProjectsTableBody = getElement('activeProjectsTableBody');
    const completedProjectsTableBody = getElement('completedProjectsTableBody');
    ...
    const updatesError = getElement('updatesError');
    const addUpdateForm = getElement('addUpdateForm');
    // const addUpdateMessage = getElement('addUpdateMessage'); // Used within its handler
    ...
    const forecastErrorMessageEl = getElement('forecast-error-message');
    const themeToggleBtn = getElement('theme-toggle-btn'); // This might be the header one
    const themeToggleBtnSidebar = getElement('theme-toggle-btn-sidebar'); // Sidebar one
    // const userDisplayNameEl = getElement('user-display-name'); // Handled in fetchUserInfo
    const logoutBtn = getElement('logout-btn'); // Header logout
    const logoutBtnNavFooter = getElement('logout-btn-nav-footer'); // Nav footer logout
    ...
    // Apply initial states that don't depend on other initializations first
    // These functions are now defined in script.js and can be called directly
    applyInitialTheme(); // Apply theme first to avoid FOUC
    applyInitialNavState(); // Apply nav state
    ...
    function initializeEventListeners() {
        ...
        logoutBtnNavFooter?.addEventListener('click', handleLogout); // Listener for nav footer logout
        console.log("    - Button click listeners added.");

        // Theme toggle from header (if it exists and is different from sidebar one)
        // Assuming 'themeToggleBtn' is the ID for a header theme toggle
        const headerThemeToggle = getElement('theme-toggle-btn'); // General ID, could be header
        if (headerThemeToggle && headerThemeToggle !== themeToggleBtnSidebar) {
            headerThemeToggle.addEventListener('click', toggleTheme);
        }
        themeToggleBtnSidebar?.addEventListener('click', toggleTheme); // Sidebar theme toggle

        navToggleBtn?.addEventListener('click', toggleNavigation); // Main nav toggle
        ...
        modalsContainer?.addEventListener('click', (event) => {
            ...
            // MODIFIED LOGIC HERE:
            // Close on 'X' button OR if it's a backdrop click AND NOT the addProjectModal
            if (isCloseButton || (isBackdropClick && modal.id !== 'addProjectModal')) {
                console.groupCollapsed(`UI: Closing Modal via ${isCloseButton ? 'Button' : (isBackdropClick ? 'Backdrop' : 'Other')}`);
                ...
                console.groupEnd();
            } else if (isBackdropClick && modal.id === 'addProjectModal') {
                console.log("UI: Backdrop click on Add Project Modal - close prevented.");
            }
        });
        console.log("    - Delegated modal close listener added (Add Project Modal backdrop click prevented).");
        ...
        const addSortListenerToTable = (tableId) => {
            ...
            thead.addEventListener('click', (event) => {
                ...
                if (currentSort.key === sortKey) {
                    ...
                    nextDirection = 'asc'; // Default to ascending for new key
                }
                ...
                thead.querySelectorAll('.sortable-header').forEach(th => {
                    ...
                    if (th === header) {
                        ...
                        if (iconSpan) iconSpan.textContent = ''; // Clear icon for non-active sort headers
                    }
                });
    ...
    try {
        ...
        setupCollapsibleSections(); // Setup general collapsible sections
        ...
        console.groupEnd(); // End of App Initialization Sequence
    }
});
